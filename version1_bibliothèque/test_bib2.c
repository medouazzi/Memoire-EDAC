...
void* fonction3(void* arg) {
   // pthread_edac_t* edac = (pthread_edac_t*)arg;
    ...
    unsigned char data_to_write2[10];
     for (int i = 0; i < 10; ++i) {
        data_to_write2[i] = 0x12*i ;  // Filling data with values 1, 2, 3, ..., 15
    }
  
    int size = pthread_edac_write((pthread_edac_t*)arg, data_to_write2, sizeof(data_to_write2), 0);
    printf("the number of characters actually written : %d \n",size);
...
int main() {
    ...
    pthread_create(&threads[1], NULL, fonction3, &edac);
    
    pthread_create(&threads[3], NULL, fonction2, &edac);
    pthread_create(&threads[4], NULL, fonction, &edac);
    
    pthread_create(&threads[5], NULL, fonction2, &edac);
