...
int myopen(struct inode *inode, struct file *filp) {
    ...
    printk(KERN_INFO "Flags parameter: 0x%x\n", flags);
    ...
    printk( "priv data  %p \n", filp->private_data );
    printk( "new priv data %p  \n", new_filp->private_data );
    // Check if EDAC is already initialized
    if (filp->private_data != NULL  ) { if ((filp->f_flags & O_CREAT) == 0) return 0;
    else { printk(KERN_ERR "EDAC memory already initialized\n");
        return -EBUSY;}
